...
const utils_1 = require("@caltrack/utils");
const prisma_1 = __importDefault(require("../src/db/prisma"));
async function main() {
    console.log('Clearing database tables...');
    await prisma_1.default.auditEvent.deleteMany({});
    await prisma_1.default.calibrationTestPoint.deleteMany({});
    await prisma_1.default.calibrationRecord.deleteMany({});
    await prisma_1.default.instrument.deleteMany({});
    await prisma_1.default.controlLoop.deleteMany({});
    await prisma_1.default.processArea.deleteMany({});
    console.log('Database cleared. Seeding plant hierarchy...');
    // 1. Create Process Areas
    const area10 = await prisma_1.default.processArea.create({
        data: {
            areaCode: '10',
    ...
    const area12 = await prisma_1.default.processArea.create({
        data: {
            areaCode: '12',
    ...
    const area15 = await prisma_1.default.processArea.create({
        data: {
            areaCode: '15',
    ...
    const loop101 = await prisma_1.default.controlLoop.create({
        data: {
            loopNumber: '101',
    ...
    const loop215 = await prisma_1.default.controlLoop.create({
        data: {
            loopNumber: '215',
    ...
    const loop301 = await prisma_1.default.controlLoop.create({
        data: {
            loopNumber: '301',
    ...
    async function seedInstrument(data) {
        ...
        // 1. Create Instrument
        const instrument = await prisma_1.default.instrument.create({
            data: {
        ...
        const outputSpan = (0, utils_1.getOutputSpan)(instrument.rangeMin, instrument.rangeMax, instrument.signalType);
        ...
        for (const cal of data.calibrations) {
            if (cal.isLegacy) {
                ...
                const record = await prisma_1.default.calibrationRecord.create({
                    data: {
                ...
                const pointsData = percentages.map((p, idx) => {
                    const targetInput = (0, utils_1.calculateTargetInput)(instrument.rangeMin, instrument.rangeMax, p);
                    const expectedOutput = (0, utils_1.calculateExpectedOutput)(instrument.rangeMin, instrument.rangeMax, instrument.signalType, p);
                    ...
                    const asFoundError = (0, utils_1.calculateErrorPercent)(asFoundOutput, expectedOutput, outputSpan);
                    const asLeftError = (0, utils_1.calculateErrorPercent)(asLeftOutput, expectedOutput, outputSpan);
